...
## 
## Move the initial position of the particles using snapshots
## to make a half-circle arch in the center (like an "H")
##
## (2022)
...
## !! Change the initial position of the particles !!
##   (move them into a half-circle to make an "H")

# get the current system (1 frame snapshot)
...
for i in range(snap.particles.N):
	...
	# (x - x.c)^2 + (y-y.c)^2 = R^2
	if abs(temp_x) < 2:
		temp_y = math.sqrt(radius**2 - temp_x**2)
		pos[1] = temp_y
# apply the changes
...
# save outputs
gsd_writer = hoomd.write.GSD(trigger=period, filename="H-Equilibrium.gsd",
